...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async () => {
    ...
    // To render the data in a person's local timezone
          ...
          return (
            <div>
              <div className='gif-item' key={i}>
                <img src={mint.url} alt={`Minted NFT ${mint.name}`} />
              </div>
              <p>{mint.name}</p>
              {mint.shu.map((item) => {
                const shuValue = item.value
                return <p>Scoville Heat Units: {shuValue}</p>
              })}
            </div>
          )
        })}
